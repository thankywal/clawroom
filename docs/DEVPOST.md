...
The surface is also live rather than fixed. Switching rooms changes what the agents in the room can do, and since the API has no `unregisterTool()` that turns entirely on the `AbortController` the tools were registered under. Registration is per tool with its own controller, so approving a source registers four tools and moves nothing else: measured going from 21 to 25 with no dip, where a whole-surface rebuild used to drop to zero in between. `ontoolchange` is what tells a long-lived client about it, and the MCP bridge listens on it and sends `notifications/tools/list_changed`.
...
And the person approving reads the thing itself. The approval card carries the headline, the words, and the arguments the call was made with, not just its one-line summary. This project's line is "the room sees the work, not the people": that is about never capturing somebody's conversation, and it was never about hiding the artefact from the person being asked to ship it. An approver who cannot read what they are approving is a rubber stamp, which is the failure mode this whole design exists to avoid.
