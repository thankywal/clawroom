...
`/selftest.html` calls every tool through `executeTool()` with no agent and passes **22 of 22** against the deployed origin in a clean Chrome, no flags. Nine of those test the claim rather than a function: a sentinel sentence written into a private draft and then hunted through all of shared state; a publish that must leave the board unmoved until a human clicks; a canary in a file on the sandbox; a page served on a port that must not reach the board until `computer_share_page`; a snapshot that survives `rm -rf`; and a borrowed refund that parks and only then takes effect.
